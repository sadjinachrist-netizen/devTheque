                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('articles.index') }}" class="flex items-center gap-2">
                        <svg width="34" height="34" viewBox="0 0 84 84" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect width="84" height="84" rx="18" fill="#2563eb"/>
                            <polyline points="34,26 22,42 34,58" fill="none" stroke="#ffffff" stroke-width="5.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <polyline points="50,26 62,42 50,58" fill="none" stroke="#ffffff" stroke-width="5.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <line x1="38" y1="38" x2="46" y2="38" stroke="#5eead4" stroke-width="3.2" stroke-linecap="round"/>
                            <line x1="38" y1="42" x2="46" y2="42" stroke="#5eead4" stroke-width="3.2" stroke-linecap="round"/>
                            <line x1="38" y1="46" x2="46" y2="46" stroke="#5eead4" stroke-width="3.2" stroke-linecap="round"/>
                        </svg>
                        <span class="text-xl font-bold text-brand-blue">DevThèque</span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-6 sm:-my-px sm:ms-10 sm:flex sm:items-center">
                    ...
                    @auth
                        ...
                        @if (auth()->user()->isLecteur())
                            <x-nav-link :href="route('author-requests.create')" :active="request()->routeIs('author-requests.*')">
                                {{ __('Devenir auteur') }}
                            </x-nav-link>
                        @endif
                        @if (auth()->user()->isAdmin())
                            <x-nav-link :href="route('admin.author-requests.index')" :active="request()->routeIs('admin.author-requests.*')">
                                {{ __('Demandes') }}
                            </x-nav-link>
                        @endif
                    @endauth
            ...
            <div class="hidden sm:flex sm:items-center sm:gap-4">
                @auth
                    @if (auth()->user()->isAuteur() || auth()->user()->isAdmin())
                        <a href="{{ route('articles.create') }}"
                           class="inline-flex items-center gap-1.5 px-4 py-2 bg-brand-blue text-white text-sm font-semibold rounded-lg hover:bg-brand-blue-dark transition">
                            + Nouvel article
                        </a>
                    @endif
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900 focus:outline-none">
                                <span class="w-9 h-9 rounded-full bg-brand-blue text-white flex items-center justify-center font-bold">
                ...
                @guest
                    ...
                    <a href="{{ route('register') }}"
                       class="px-4 py-2 bg-brand-blue text-white text-sm font-semibold rounded-lg hover:bg-brand-blue-dark transition">
                        {{ __('Inscription') }}
                    </a>
                @endguest
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden border-t border-slate-200">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                ...
                @endif
                @if (auth()->user()->isLecteur())
                    <x-responsive-nav-link :href="route('author-requests.create')" :active="request()->routeIs('author-requests.*')">
                        {{ __('Devenir auteur') }}
                    </x-responsive-nav-link>
                @endif
                @if (auth()->user()->isAdmin())
                    <x-responsive-nav-link :href="route('admin.author-requests.index')" :active="request()->routeIs('admin.author-requests.*')">
                        {{ __('Demandes') }}
                    </x-responsive-nav-link>
                @endif
            @endauth
